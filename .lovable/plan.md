...
- Formular zum Anlegen und Bearbeiten direkt auf der Seite (Dialog):
  - Name (Kontoinhaber/Bezeichnung) – Pflicht
  - IBAN – Pflicht
  - BIC – optional
  - Bankname – optional
  - Limit in Euro – Pflichtfeld, vorbelegt mit 20.000 €
...
- Migration: Tabelle `public.bank_accounts` mit `id`, `name`, `iban`, `bic`, `bank_name`, `limit_amount numeric NOT NULL DEFAULT 20000`, `is_active boolean NOT NULL DEFAULT true`, `created_by`/`updated_by`, `created_at`/`updated_at` inkl. Update-Trigger. GRANTs für `authenticated` (SELECT/INSERT/UPDATE/DELETE) und `service_role`, RLS aktiv, alle Policies über `has_role(auth.uid(), 'admin')`.
- `src/lib/bank-accounts.functions.ts`: `listBankAccounts`, `saveBankAccount`, `deleteBankAccount` als `createServerFn` mit `requireSupabaseAuth` + Admin-Prüfung via `supabase.rpc("has_role", ...)`, Zod-Validierung (IBAN normalisiert auf Großbuchstaben ohne Leerzeichen, Limit ≥ 0).
